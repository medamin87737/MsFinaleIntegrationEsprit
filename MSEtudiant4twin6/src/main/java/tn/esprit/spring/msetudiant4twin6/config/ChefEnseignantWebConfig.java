...
public class ChefEnseignantWebConfig implements WebMvcConfigurer {
    ...
    private static final String RESOURCE_BASE = "/etudiants";
    ...
    public HandlerInterceptor chefEnseignantInterceptor() {
        return new HandlerInterceptor() {
            ...
            public boolean preHandle(
                ...
                if (!underEtudiants(path) || "OPTIONS".equalsIgnoreCase(request.getMethod())) {
                    return true;
                }
                if ("GET".equalsIgnoreCase(request.getMethod())) {
                    ...
                    response.getWriter().write(
                            "{\"message\":\"Accès GET réservé à Enseignant/Chef Enseignant ou à un utilisateur JWT avec rôle realm.\"}");
                    return false;
                }
                if (isChef(request)) {
                    return true;
    ...
    public void addInterceptors(@NonNull InterceptorRegistry registry) {
        registry.addInterceptor(chefEnseignantInterceptor())
                .addPathPatterns("/etudiants/**")
                .excludePathPatterns("/etudiants/auth/**", "/etudiants/portail", "/etudiants/portail/**");
    }
    ...
    private static boolean underEtudiants(String path) {
