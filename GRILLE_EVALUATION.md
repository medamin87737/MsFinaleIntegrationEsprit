...
|------------------|---------------|-------------------------|---------------------|
| **Micro-service techno avancée + BD** | 2 | **`MSNotes4twin6`** (NestJS) + **MongoDB** (compose + config `MSNotes4twin6.properties`) | Expliquer choix Nest + Mongoose ; montrer `package.json` et URI Mongo. |
...
| **Docker Compose** | 2 | **`docker-compose.yml`** (inclut la stack complète) + **`docker-compose.full-stack.yml`** + **Dockerfiles** ; **`aws/DEPLOY-AWS.md`** pour le cloud | `docker compose up -d --build` dans `MS/`. |
...
| **Valeurs ajoutées** | 2 | **CI/CD** : **`.github/workflows/ci.yml`** ; **Prometheus/Grafana** : **`docker-compose.monitoring.yml`**, **`monitoring/`** ; **K8s** : **`k8s/`** ; **AWS** : **`aws/DEPLOY-AWS.md`** | Captures GitHub Actions vertes ; Grafana ; démo EC2/ECS ou schéma AWS. |
