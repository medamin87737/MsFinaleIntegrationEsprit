package tn.esprit.spring.msenseignant4twin6.keycloak;
...
import tn.esprit.spring.msenseignant4twin6.RoleEnseignant;

/**
 * Crée / met à jour / supprime les utilisateurs du realm Keycloak alignés sur les fiches enseignants,
 * pour que le portail (login Keycloak + claims {@code school_enseignant_id}) fonctionne.
 */
@Service
...
public class KeycloakUserProvisioningService {
    ...
    public KeycloakUserProvisioningService(RestTemplate restTemplate) {
        this.restTemplate = restTemplate;
    }

    /**
     * @param password requis à la création ; à la mise à jour, si vide on ne change pas le mot de passe Keycloak.
     */
    public void syncEnseignant(
            String matricule,
            String password,
            Long schoolEnseignantId,
            RoleEnseignant role,
            boolean creating) {
        ...
        if (schoolEnseignantId == null) {
            throw new IllegalArgumentException("Identifiant enseignant manquant.");
        }
        String token = getAdminAccessToken();
        String roleName = role == RoleEnseignant.CHEF_ENSEIGNANT ? "ROLE_CHEF_ENSEIGNANT" : "ROLE_ENSEIGNANT";
        ...
        attrs.put("schoolEnseignantId", List.of(String.valueOf(schoolEnseignantId)));
        ...
        if (userIdOpt.isEmpty()) {
            ...
            replaceRealmRoles(token, userId, roleName);
            log.info("Keycloak: utilisateur enseignant créé (username={})", matricule);
            return;
        }
        ...
        replaceRealmRoles(token, userId, roleName);
        log.info("Keycloak: utilisateur enseignant mis à jour (username={})", matricule);
    }
    ...
    public void deleteByMatricule(String matricule) {
        ...
        try {
            ...
            log.info("Keycloak: utilisateur {} supprimé", matricule);
        } catch (Exception e) {
            log.warn("Keycloak: suppression impossible pour {} — {}", matricule, e.getMessage());
        }
    }

    private void replaceRealmRoles(String token, String userId, String primaryRole) {
        String[] managed = {"ROLE_ENSEIGNANT", "ROLE_CHEF_ENSEIGNANT", "ROLE_ETUDIANT"};
        HttpHeaders h = new HttpHeaders();
        h.setBearerAuth(token);
        ...
        JsonNode roleNode = getRealmRole(token, primaryRole);
        if (roleNode == null || !roleNode.has("id")) {
            throw new IllegalStateException("Rôle realm introuvable: " + primaryRole);
        }
